...
<div class="box">
    ...
    <div class="box-item">
        ...
        ${assignment.quiz.time} <fmt:message key="min" bundle="${bundle}"/>
    </div>
    <div class="box-item">
        ...
        ${assignment.quiz.numberOfQuestion} <fmt:message key="questions_short" bundle="${bundle}"/>
    </div>
    <c:if test="${assignment.score != 0 && assignment.status == AssignmentStatus.Completed}">
    <div class="box-item">
        <div class="ScoreBar">
            <div class="ScoreBarPe" style="width: ${assignment.score}%;"></div>
        </div>
        <div class="score">&nbsp;${assignment.score}%</div>
    </div>
    </c:if>
    <c:if test="${assignment.status == AssignmentStatus.NotStarted && sessionScope.blocked == null}">
        <a class="btn btn-outline-success start" data-id="${assignment.quizId}"
           href="${pageContext.request.contextPath}/controller/user/assignments/assignment?quizId=${assignment.quizId}"> Start </a>
    </c:if>
    <c:if test="${assignment.status != AssignmentStatus.NotStarted}">
        <div class="box-item status"><fmt:message key="${assignment.status}" bundle="${bundle}"/></div>
    </c:if>
</div>
